...
def make_particle_projections(in_vol: str,
                              in_coords: str,
                              out_dir: str,
                              extract_shape: tuple,
                              voxel_spacing: float,
    ...
    while True:
        ...
        if len(fnames) > 0:
            ...
            if len(fnames) == 1 and os.path.splitext(in_coords)[-1] == '.json':
                ...
                coords = read_starfile(in_coords, 
            ...
            for run_name in coords.keys():
                if load_method == 'copick':
                    volume = cp_interface.get_run_tomogram(run_name, 
                                                           voxel_spacing, 
                                                           tomo_type)
                if load_method == 'mrc':
...
def generate_from_starfile(in_vol: str,
                           in_coords: str,
                           out_dir: str,
                           extract_shape: tuple,
                           voxel_spacing: float,
...
def old_generate_from_starfile(vol_path: str,
