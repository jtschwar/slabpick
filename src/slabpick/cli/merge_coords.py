...
from slabpick.coordinates import consolidate_coordinates_sets
from slabpick.dataio import make_starfile, read_starfile
from slabpick.visualize import visualize_merge_pairs


def parse_args():
    """Parser for command line arguments."""
    parser = ArgumentParser(description="Generate starfile based on cryosparc-curated picks.")
    parser.add_argument("--in_star1", type=str, required=True, help="Starfile containing first set of coordinates")
...
def main(config):
    ...
    d_coords_merge, d_clusters = consolidate_coordinates_sets(coords1, coords2, config.threshold, config.weights)

    make_starfile(d_coords_merge, config.out_star, coords_scale=1.0 / config.coords_scale)

    if config.copick_json is not None and config.out_visuals is not None:
        os.makedirs(config.out_visuals, exist_ok=True)
        for run_name in d_clusters:
            if len(d_clusters[run_name]) != 0:
                visualize_merge_pairs(
                    run_name,
                    coords1,
                    coords2,
                    d_clusters,
